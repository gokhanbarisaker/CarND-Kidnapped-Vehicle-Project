 ...
 */
inline double gaussian2d(double const &mu_x, double const &mu_y, double const &sigma_x, double const &sigma_y, double const &x, double const &y)
{
  // Exponential part
  double ep1 = std::pow(x - mu_x, 2.0) / std::pow(sigma_x, 2.0);
  double ep2 = std::pow(y - mu_y, 2.0) / std::pow(sigma_y, 2.0);
  double ep = std::exp(-0.5 * (ep1 + ep2));

  return ep / (M_2_PI * sigma_x * sigma_y);
}
